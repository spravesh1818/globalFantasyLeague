...
def create_user(db: Session, user: schemas.UserCreate):
    db_user = models.User(username=user.username, email=user.email, full_name=user.full_name,
                          disabled=user.disabled, hashed_password=user.password, role=user.role)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def create_player(db: Session, player: schemas.Player):
    db_player = models.Player(name=player.name, nationality=player.nationality,
                              kit_no=player.kit_no, dominant_foot=player.dominant_foot)
    db.add(db_player)
    db.commit()
